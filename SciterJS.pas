...
uses
  Windows, Messages, Classes, SysUtils, Contnrs, Variants, Math, Generics.Collections,
  SciterJSAPI;
...
type
  ...
  TSciter = class(TInterfacedPersistent, _ISciterEventHandler)
  private
    FHandle: HWINDOW;
    FBaseUrl: WideString;
  ...
  public
    constructor Create(Window: HWINDOW = 0);
    destructor Destroy; override;
    ...
    function Call(const FunctionName: WideString; const Args: TArray<Variant> = []): Variant;
    procedure Fire(he: HELEMENT; cmd: UINT; data: Variant; async: Boolean = True);
    procedure FireCustom(he: HELEMENT; name: String; data: Variant; async: Boolean = True);
    ...
    procedure Refresh;
    property Handle: HWINDOW read GetHandle;
  ...
  TNativeClassProc = procedure(argc: UINT; argv: PSciterValue; retval: PSciterValue) of object; cdecl;
  PNativeClassProc = ^TNativeClassProc;
  ...
  public
    class var Methods: TObjectDictionary<String, TDictionary<String, TNativeClassProc>>;

    class procedure AddMethod(Name: String; Method: TNativeClassProc);
    class procedure RegisterMethods(var ReturnValue: TSciterValue); virtual;
  end;
...
function WindowDelegate(hwnd: HWINDOW; msg: UINT; wParam: WPARAM; lParam: LPARAM; pParam: LPVOID; var pbHANDLED: BOOL): LRESULT; stdcall;
begin
  Result := API.SciterProcND(hwnd, msg, wParam, lParam, pbHANDLED);
end;

function CreateInvisibleWindow: HWINDOW;
var
  Frame: TRect;
begin
  Frame.Left := -9999;
  ...
  Frame.Width := 0;
  Frame.Height := 0;
  Result := API.SciterCreateWindow(SW_MAIN, Frame, @WindowDelegate, nil, 0);
end;

constructor TSciter.Create(Window: HWINDOW = 0);
var
  SR: SCDOM_RESULT;
begin
  ...
  if Window = 0 then
    Window := CreateInvisibleWindow;
  if Window = 0 then
    SciterError('Cannot find window to embed Sciter in!');
  FHandle := Window;
...
function TSciter.Call(const FunctionName: WideString; const Args: TArray<Variant> = []): Variant;
begin
...
procedure TSciter.FireCustom(he: HELEMENT; name: String; data: Variant; async: Boolean = True);
var
  evt: BEHAVIOR_EVENT_PARAMS;
  bHandled: BOOL;
  pVal: TSciterValue;
begin
  evt.cmd := BEHAVIOR_EVENTS.CUSTOM;
  API.ValueInit(@pVal);
  V2S(data, @pVal);
  evt.data := pVal;
  evt.he := he;
  evt.heTarget := he;
  evt.name := LPCWSTR(name);
  API.SciterFireEvent(evt, async, bHandled);
  API.ValueClear(@pVal);
end;
...
function TSciter.GetHandle: HWINDOW;
begin
  Result := FHandle;
end;

function TSciter.HandleAllocated: Boolean;
begin
...
begin
  {
  if Element = nil then
    SciterJSAPI.SciterWindowInspector(Handle, API)
  else
    SciterJSAPI.SciterInspector(HELEMENT(Element.Handle), API);
  }
end;
...
class procedure TNativeMethods.AddMethod(Name: String; Method: TNativeClassProc);
var
  ClassMethods: TDictionary<String, TNativeClassProc>;
begin
  if Methods.TryGetValue(Self.ClassName, ClassMethods) then
    ClassMethods.Add(Name, Method)
  else
  begin
    ClassMethods := TDictionary<String, TNativeClassProc>.Create;
    ClassMethods.Add(Name, Method);
    Methods.Add(Self.ClassName, ClassMethods);
  end;
end;

class procedure TNativeMethods.RegisterMethods(var ReturnValue: TSciterValue);
var
  Last: Integer;
begin
...
initialization
  ...
  TNativeMethods.Methods := TObjectDictionary<String, TDictionary<String, TNativeClassProc>>.Create([doOwnsValues]);

finalization
  if Assigned(Behaviors) then
    Behaviors.Free;
  for var Pair in PackedRes do
    if Assigned(pair.Value) then
      API.SciterCloseArchive(pair.Value);
  PackedRes.Clear;
  FreeAndNil(PackedRes);
  TNativeMethods.Methods.Free;
